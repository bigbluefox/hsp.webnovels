...
namespace Hsp.Novels.DbUtility
{
    ...
    public class SQLiteHelper
    {
        ...
        public static string ConnectionString = ConfigurationManager.AppSettings["sqlite"] ?? "";
        ...
        public static bool Exists(string strSql, params SQLiteParameter[] commandParameters)
        {
            object obj = GetSingle(strSql, commandParameters);
            int cmdresult;
            if ((Equals(obj, null)) || (Equals(obj, DBNull.Value)))
            {
                cmdresult = 0;
        ...
        /// <returns>查询结果（object）</returns>
        public static object GetSingle(string sqlString, params SQLiteParameter[] commandParameters)
        {
            using (var connection = new SQLiteConnection(ConnectionString))
            {
                using (var cmd = CreateCommand(connection, sqlString, commandParameters))
                {
                    try
                    {
                        connection.Open();
            ...
            //    {
            //        AttachParameters(cmd, sqlString, commandParameters);
            //    }
        ...
        public static SQLiteCommand CreateCommand(string connectionString, string commandText,
            params SQLiteParameter[] commandParameters)
        {
        ...
        public static DataSet ExecuteDataSet(string connectionString, string commandText, object[] paramList)
        {
            ...
            //if (da.SelectCommand.UpdatedRowSource != UpdateRowSource.None)
            //{
            //    da.Fill(ds);
            //}
        ...
        ///     SQLiteTransaction. Transaction consists of Connection, Transaction,  /// and Command, all of
            ...
            params SQLiteParameter[] commandParameters)
        {
            ...
            var ds = ExecuteDataset((SQLiteCommand) cmd);
            return ds;
        }
        ...
        /// </summary>
        /// <param name="transaction">
        ///     SQLiteTransaction. Transaction consists of Connection, Transaction,    /// and Command, all
        ...
        public static void UpdateDataset(SQLiteCommand insertCommand, SQLiteCommand deleteCommand,
            SQLiteCommand updateCommand, DataSet dataSet, string tableName)
        {
...
/*
XCode里面，SQLite会默认采用最高性能的配置。
普通的SQLite连接字符串如下：
<add name="SQLite" connectionString="Data Source=test.db;Version=3;" providerName="Sqlite"/>

在XCode里面，会自动加上几个参数，变成：
<add name="SQLite" connectionString="Data Source=test.db;Version=3;Pooling=true;Cache Size=50000;Synchronous=Off;Journal Mode=Memory" providerName="Sqlite"/>
Pooling=true，打开连接池
Cache Size=50000，把缓存大小设到最大
Synchronous=Off，关闭同步，避免SQLite每次修改数据后都确认是否已经写入磁盘
Journal Mode=Memory，把临时文件放到内存

这四个参数是提高SQLite性能的利器，特别是后面两个，同时它们俩也是争议最大的！
最近两个月，一个客户的系统（采用SQLite）因为突然断电导致多次SQLite数据库出错，确认是由于上面后两个设置所致！
*/